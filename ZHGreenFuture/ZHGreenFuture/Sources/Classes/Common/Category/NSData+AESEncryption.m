...
+ (NSString*)aesBase64encode:(NSString *)str
{
    if ([str length] == 0)
        return @"";
    const char *source = [str UTF8String];
    int strlength  = strlen(source);
    char *characters = malloc(((strlength + 2) / 3) * 4+1);
    memset(characters, 0, ((strlength + 2) / 3) * 4+1);
    if (characters == NULL)
        return nil;
    NSUInteger length = 0;
    NSUInteger i = 0;
...
//    NSString *g = [[NSString alloc] initWithBytes:characters length:length encoding:NSASCIIStringEncoding];
